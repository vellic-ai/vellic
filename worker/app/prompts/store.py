...
async def list_overrides(conn: asyncpg.Connection, repo_id: str) -> list[dict]:
    ...
    rows = await conn.fetch(
        "SELECT path, body, updated_at FROM prompt_overrides WHERE repo_id = $1 ORDER BY path",
        repo_id,
    )
...
async def get_override(
    ...
    row = await conn.fetchrow(
        "SELECT path, body, updated_at FROM prompt_overrides WHERE repo_id = $1 AND path = $2",
        repo_id,
        path,
    )
