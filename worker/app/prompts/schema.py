"""Front-matter schema validation for prompt DSL files (VEL-110)."""
...
# Front-matter keys we recognise; unknown keys are rejected.
...
def validate_frontmatter(raw: dict[str, Any], source_hint: str = "") -> PromptFrontmatter:
    """Validate *raw* YAML front-matter and return a :class:`PromptFrontmatter`.

    Raises :class:`PromptValidationError` with a descriptive message on failure.
    """
    ...
    for k, v in variables_raw.items():
        if not isinstance(k, str):
            raise PromptValidationError(
                f"Variable key {k!r} must be a string{loc}"
            )
